...
+ (void)initSDKWithConfiguration:(NSDictionary *)configuration completionHandler:(OMMediationAdapterInitCompletionBlock)completionHandler{
    ...
    if(ironsourceClass && [ironsourceClass respondsToSelector:@selector(initISDemandOnly:adUnits:)]){
        [ironsourceClass initISDemandOnly:key adUnits:@[IS_REWARDED_VIDEO,IS_INTERSTITIAL]];// Interstitial && RewardedVideo Demand Only mode
        //[ironsourceClass initWithAppKey:key adUnits:@[IS_BANNER]];//Banner use Mediation API
