...
+ (void)requestCampaignWithPid:(NSString *)pid size:(CGSize)size actionType:(NSInteger)actionType payload:(NSString*)payload completionHandler:(clCompletionHandler)completionHandler {
    if (OM_STR_EMPTY(pid)) {
        OMLogD(@"adn cl p empty");
        return;
    }
    NSMutableDictionary *parameters = [NSMutableDictionary dictionaryWithDictionary:[self clParametersWithPid:pid size:size actionType:actionType payload:payload]];
    NSError *jsonError;
...
+ (NSDictionary*)clParametersWithPid:(NSString*)pid size:(CGSize)size actionType:(NSInteger)actionType payload:(NSString*)payload {
